    ...
    public ChessGame(QuickChess gameIn) {
      ...
      for(int y=7;y>-1;y--){
        ...
        for(int x=0;x<8;x++){
          ...
          if(odd){
            buttons[x][y].setBackground(new java.awt.Color(100, 50, 0));//150,100,50
          }else{
            buttons[x][y].setBackground(new java.awt.Color(255, 214, 160));//255, 204, 150
          }
          odd=!odd;
    ...
    public void updateBoard(){
      for(int x=0;x<8;x++){
        for(int y=0;y<8;y++){
          ...
          try{
            buttons[x][y].setIcon(new javax.swing.ImageIcon(getClass().getResource(game.board[x][y].piece.image())));
          }catch(Exception e){
